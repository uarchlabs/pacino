...
Two consumers are affected, and they are not equally forgiving:

- THE RAS RETURN ADDRESS. ras_decisions.md 8 names the FTB
  fallThroughAddr as the source of the pushed return address and says
  the RAS does not compute PC+2 or PC+4 itself. The FTQ has no field
  holding it, so a block terminated by a call pushes an address past
  the call. Cost is prediction accuracy on the matching return.
- THE NOT-TAKEN SUCCESSOR. ftq_entry_formats.md 2 stores this field
  precisely so the successor can be re-derived after a redirect
...
6. CLOSED, PA-direct correction, session-064. Section 2.2 and section 9 placed
...
7. CLOSED, PA-direct correction, session-064. Section 3.1 named redirect
...
```
  2026-08-21  BP-107 results. Section 4: bp_ftq_entry_t.pft_addr is
              ...
              scalar. On a uBTB miss it is the full block end, which
              breaks both the RAS return address (ras_decisions.md 8)
              and the not-taken successor. The fix belongs in the p2
              group, not in a new entry field. Section 4b added: no
              p1 port carries the block start PC, so the FTQ stages
              its own request to write its own entry.

  2026-08-21  Section 9 said FOUR bytes per position while section
              7.4 of this same file said two. POS_OFFSET_BITS is 1;
              two is correct, stale by BP-099. Section 9's claim
              that bp_history folds pred_pc[3] and [2] is
              unaffected -- that is a fact about pred_pc, not about
              the multiplier -- but the pos-relative identity it
              implies changes; see PROJECT_STATUS, bp_history
              decomposition.
              Also: three citations of INFRA-012, in section 10
              items 6 and 7 and in this history, were a label
              attached retroactively to the session-064 PA-direct
