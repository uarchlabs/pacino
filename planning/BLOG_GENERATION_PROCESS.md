...
1. Blog output file name format
    ...
    This is the authoring file name. The publishing pipeline renames the
    file on publish to a dated name in a separate repository. Series
    navigation links target the published name, not this one -- see
    Navigation section markers and Indexability / Internal linking.
...
The PA emits these markers with the ::BEGIN LINKS:: / ::END LINKS:: block
empty. The PA does not fill in series navigation links. The links cannot be
generated at authoring time: a post's published file name encodes a publish
date that has not occurred yet, its "next" link targets a post that may not
be written yet, and the target files live in a separate repository the
drafting session does not have. Navigation is generated by the publishing
pipeline at rename/publish time, which is the first point at which all posts,
their final dated names, and their paths coexist. See Indexability / Internal
linking for the anchor-text rule the pipeline applies.
...
### Catch-up posts and TD status

The blog series may be written behind the design work, so a post can cover a
session range whose technical debt has since changed state. PROJECT_STATUS.md
and CLOSED_TECH_DEBT.md carry current status, not the status as of the post's
range. Copied verbatim, the table can then show items as closed by experiments
that run after the post's range and are never mentioned in the post, which
misreports what the range did.

For a catch-up post, the Technical Debt Referenced table reports status as of
the close of the post's range, not current status. Roll each entry back to its
end-of-range state and add a note above the table stating that later
experiments outside the range have since changed some items. This is a
deliberate deviation from the "copied verbatim" rule above, for posts whose
range predates the current status files.
...
Each post covers a contiguous block of PA sessions and answers three
questions:
...
Note on terminology: "series" in this document means the sequence of
published blog posts (the bpu series, etc.). It does not refer to the range
of PA sessions or experiments under review in a given post; that is called
the "range." See Step 8, which uses "range" for this reason.
...
The generalization is checked across the clusters in the range under review.
It is not required to be checked against, or to avoid repeating, the
generalization of a prior blog post. "Series" elsewhere in this document
refers to the published post sequence; this step operates on the range.
...
- Series navigation links are generated by the publishing pipeline, not by
  the PA at authoring time. The PA leaves the ::BEGIN LINKS:: / ::END LINKS::
  block empty (see Navigation section markers).
- When the pipeline generates the links, the anchor text must name the post
  ("Part 5: TAGE — Architecture and the Decomposition Problem"), not use
  generic anchor text ("here," "previous post"). The post's TITLE metadata
  field is the source for this anchor text. Cross-links between posts are a
  search ranking signal; descriptive anchor text carries more weight than
  generic anchor text.
- Link targets are the published, dated file names in the publishing
  repository, not the BLOG_[series]_[number]_[short_description].md authoring
  names. The pipeline holds both, ordered by publish date; each post's
  previous/next are its date-adjacent neighbors.
