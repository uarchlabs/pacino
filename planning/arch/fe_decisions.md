...
NUM_PRED_SLOTS predictions, one per prediction slot. Slot assignment is
by fixed PC range; see section 10.
...
The selection is made per prediction slot; each slot resolves
independently.
...
current block is fetched. When no slot carries a prediction, fetch
proceeds sequentially at PC + fetch_width until a later predictor
redirects the entry.
...
one fetch block and is the value carried in `branch_id`. All
...
The block's successor PC is selected across slots in slot order:

```
  slot 0 valid and taken   ->  slot 0 target
  slot 1 valid and taken   ->  slot 1 target
  otherwise                ->  fallthrough address
```

Slot order is PC order. Slot 0 covers the lower address range of the
block, so a taken branch in slot 0 means control leaves the block
before slot 1 is reached and slot 1 is off the executed path. The
selection is re-evaluated whenever a redirect rewrites a slot.

At most one slot supplies the successor, so at most one branch per
block is on the executed path.
...
index and one prediction slot, and carries a corrected target.
...
```
  <pred>_redir_val_<pN>  [0:NUM_PRED_SLOTS-1]  redirect asserted
  <pred>_redir_tgt_<pN>  [0:NUM_PRED_SLOTS-1]  corrected target PC,
                                               VA_WIDTH
  <pred>_redir_ftq_idx_<pN>                    FTQ entry index being
                                               corrected
```

The slot being corrected is the array index. There is no slot
identifier field. A predictor may assert a redirect on more than one
slot in the same cycle; TAGE and SC both predict every slot in one
cycle, so a block with a conditional branch in each slot can produce
two redirects from one predictor at one stage.

`ftq_idx` is scalar. Both slots belong to one fetch block, which is one
FTQ entry, so one entry index covers every slot the predictor corrects
in that cycle.
...
Both are per slot. Each slot carries its own `br_type`, so slot 0 and
slot 1 of one block may be corrected by different predictors in the
same cycle.
...
branch type of each slot, at p2. That type selects the target source
for that slot:
...
A redirect rewrites one slot. The block's successor PC is re-derived
from the updated slot set per section 2.4.
...
Supersession does not cross slots. A p3 redirect on slot 1 does not
discard a p2 redirect on slot 0 of the same entry; the two correct
different branches and both stand.
...
Array direction follows the package convention: descending
`[NUM_PRED_SLOTS-1:0]` inside packed structs, matching
`sc_pred_meta_t.sc_upd_idx`; ascending `[0:NUM_PRED_SLOTS-1]` on ports,
matching TD#90 and TD#91.

### 4.1 Fast path: bp_ftq_entry_t

Read every cycle. Written at p1 and rewritten by redirect.

Block scalar fields, one copy per entry:
...
The per-slot array is declared inside `bp_ftq_entry_t`. This departs
from the convention stated in the `bp_update_t` header comment, where
the slot array is declared at instantiation. Declaring it outside the
FTQ entry would produce one entry per slot rather than one entry per
fetch block, which is the opposite of the entry-to-block mapping stated
in section 10.

`br_type` is per slot and determines which predictors receive an update
for that slot at resolution. The set is a property of the branch class,
not of which predictors produced a result. See section 7.2.

The history pointers and the RAS snapshot are block scalar. Their
granularity is discussed in sections 6.1 and 9.
...
described in `bp_cluster.md`, FTQ Entry Split. The struct nests one
metadata block per predictor: `tage_pred_meta_t`, `sc_pred_meta_t`,
`bp_loop_meta_t`, and `ittage_pred_meta_t`.

The metadata is per slot. Each slot predicts its own branch and
produces its own provider indices, counter snapshots, and allocation
targets, so the whole structure is carried
`[NUM_PRED_SLOTS-1:0]`.
...
prediction occupies, in the slot the prediction occupies.
...
The checkpoint is block granular, one per FTQ entry, not one per slot.
This matches G20: the dual-slot history update is combined
slot-0-then-slot-1 with a bundle checkpoint, and restore anchors at the
checkpoint and replays with the branch count for the corrected path.
...
`upd_ch[NUM_PRED_SLOTS-1]`. The channel index is the slot index. Each
channel carries both conditional and indirect resolution. They are not
split by branch type.

### 7.2 Formation

On resolution of a branch at FTQ entry index k, slot s, the FTQ reads
`bp_ftq_meta_t[k]` slot s from the slow-path SRAM, and reads the
fast-path entry for `pc` and for slot s `br_type`.

The set of predictors updated is determined by that slot's `br_type`:
...
A slot that is off the executed path never resolves and forms no
update. This is ordinary speculation and requires no tracking in the
FTQ.
...
One snapshot per FTQ entry is sufficient under dual slot. At most one
slot supplies the block's successor (section 2.4), so at most one
branch per block is on the executed path and at most one RAS operation
occurs per block. A call in slot 0 and a return in slot 1 cannot both
execute: a taken call in slot 0 ends the block.
...
The 64-entry FTQ holds 64 fetch blocks and up to 64 * NUM_PRED_SLOTS
predicted branches.

Slot assignment is by fixed PC range, per G8 and G17:
...
The slot 1 PC is always pred_pc + 32. It is static and does not depend
on the slot 0 prediction.
...
```
  ...
  FE-10  One FTQ entry holds one fetch block and all NUM_PRED_SLOTS
         predictions for it. Slot assignment is by fixed PC range.
         The slot is the array index; no slot identifier field is
         carried.

  FE-11  At most one slot supplies the block's successor PC. Slots
         are selected in slot order, which is PC order. At most one
         branch per block is on the executed path.
```
...
Prior revisions of this document stated in section 10 that the second
prediction slot predicts the branch at the first slot's predicted
target. That contradicted G8 and G17, which fix slot 1 at pred_pc + 32,
static and independent of the slot 0 prediction. Section 10 now follows
G8 and G17.
...
```
  ...
  2026-07-23  fe_decisions.md. Renamed from tmp_004.md. Dual-slot FTQ
              ...
              slot. Section 2.4 successor PC selection added. Section
              10 corrected to G8/G17 fixed-range slot assignment; one
              FTQ entry now holds one fetch block. History checkpoint
              and RAS snapshot confirmed block granular, with the
              one-RAS-operation-per-block argument recorded in
              section 9. FE-10, FE-11, TD-FE-5, and FE-U9 added.
```
