...
Status: these are bp_cluster-level redirect signals, generated by
comparing each predictor's p2/p3 meta output against the FTQ-held
prediction for that fetch block. They are not present as ports on
the current unit-level tage.sv/sc.sv/ftb.sv/ittage.sv RTL (session-
060 audit, INFRA-009). They come into existence as bp_cluster top-
level wiring/logic when the cluster is built -- do not expect to
find them on any predictor unit in isolation.
